...
export SL=$(oci network security-list list  --compartment-id=$COMPARTMENT_ID --vcn-id=$VCN_ID --output table --query 'data [*].{Name:"display-name", id:id}' | grep -i aul-ras-rule )
